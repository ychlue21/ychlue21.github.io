...
$berkas = mysqli_query($koneksi, "
    ...
    join rw on warga.alamat LIKE CONCAT('%', rw.nama_rw, '%') join ketua_rw on ketua_rw.idrw=rw.idrw
    join rt on rt.idrw=rw.idrw join ketua_rt on ketua_rt.idrt=rt.idrt 
     WHERE ketua_rw.iduser = '$iduser' AND berkaspengajuansuratkeluar.idberkassurat = '$idberkassurat'
");
...
$redirectUrl = (trim(strtolower($p->status_rt)) == 'diproses') ? 'daftarpengajuanwarga.php' : 'pengajuanwarga.php';
?>

<main class="content">
    <div class="box"
        style="width: 100%; max-width: 800px; margin: auto; font-size: 20px; border: 1px; border-radius: 8px; padding: 20px;">
        <h2 class="title text-center" style="margin-top: 0;">Detail Pengajuan Warga</h2>
