...
$totalDataQuery = mysqli_query($koneksi, "SELECT COUNT(*) AS total FROM `berkaspengajuansuratkeluar`
    ...
    <table class="table table-bordered border-dark ">
        ...
        <tbody>
           ...
           $nomor = $offset + 1;
                if (isset($_GET['cari'])) {
                    $cari = $_GET['cari'];
                    $berkas = mysqli_query($koneksi, "SELECT * FROM `berkaspengajuansuratkeluar` join jenissurat on jenissurat.idJenisSurat=berkaspengajuansuratkeluar.Idjenisurat join warga on warga.NIK=berkaspengajuansuratkeluar.NIK join rw on warga.alamat LIKE CONCAT('%', rw.nama_rw, '%') join ketua_rw on ketua_rw.idrw=rw.idrw  where ketua_rw.iduser ='$iduser' and status_rt='diproses' and tanggalpengajuan like '%" . $cari . "%' ORDER BY tanggalpengajuan DESC
                    LIMIT $limit OFFSET $offset");
                } else {
                    $berkas = mysqli_query($koneksi, "SELECT * FROM `berkaspengajuansuratkeluar` join jenissurat on jenissurat.idJenisSurat=berkaspengajuansuratkeluar.Idjenisurat join warga on warga.NIK=berkaspengajuansuratkeluar.NIK join rw on warga.alamat LIKE CONCAT('%', rw.nama_rw, '%') join ketua_rw on ketua_rw.idrw=rw.idrw  where ketua_rw.iduser ='$iduser' and status_rt='diproses' ORDER BY tanggalpengajuan DESC
                    LIMIT $limit OFFSET $offset");
                }
            ...
            <tr>
                ...
                <td class="text-center"><?= ucwords(strtolower($p['tanggalpengajuan'])) ?></td>
