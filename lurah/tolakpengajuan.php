...
$statusSurat = ($status_surat === 'Ditolak') ? 'ditolak' : 'diproses';


    // Update status dan simpan keterangan ke database
    $update = mysqli_query($koneksi, "
        UPDATE berkaspengajuansuratkeluar 
        SET keterangan = '$keterangan' ,tanggalselesai='$tanggalselesai',status_surat = '$status_surat',is_viewed = 0
        WHERE idberkassurat = '$idberkassurat'
    ");

    if ($update) {
        echo "<script>alert('Keterangan telah disimpan'); window.location='suratkeluar.php';</script>";
    } else {
